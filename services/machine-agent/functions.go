...
func CheckQueue(cqr checkqueueRequest, checkqueue_url string) (checkqueueResult, error) {

	var cqrout checkqueueResult

	cqr_json, _ := json.Marshal(cqr)
	cqr_json_byte := []byte(cqr_json)

	client := &http.Client{}

	req, err := http.NewRequest("POST", checkqueue_url, bytes.NewBuffer(cqr_json_byte))
	if err != nil {
		log.Println("NewRequest: ", err)
		return checkqueueResult{}, errors.New("HTTP Post failed")
	}
...
func ExecuteBot(executionPath string, executionId string, updateStatusResultUrl string) error {
	...
	usr_json, _ := json.Marshal(usr)
	usr_json_byte := []byte(usr_json)

	client := &http.Client{}

	req, err := http.NewRequest("POST", updateStatusResultUrl, bytes.NewBuffer(usr_json_byte))
	if err != nil {
		log.Println("NewRequest: ", err)
